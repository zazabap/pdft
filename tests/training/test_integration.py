...
def test_full_training_loop_no_nan():
    ...
    for t_init, t_trained in zip(basis.tensors, result.basis.tensors):
        manifold = pdft.manifolds.classify_manifold(t_init)
        if isinstance(manifold, pdft.manifolds.UnitaryManifold):
            d = t_trained.shape[0]
            I_mat = jnp.eye(d, dtype=t_trained.dtype)
            assert jnp.allclose(t_trained @ jnp.conj(t_trained).T, I_mat, atol=1e-6)
        elif isinstance(manifold, pdft.manifolds.PhaseManifold):
...
def test_public_api_is_re_exported():
    ...
    expected = {
        # Slim public re-export hub. Less-used names live in subpackages
        # (pdft.manifolds, pdft.io, pdft.profiling, pdft.bases.circuit, ...).
        "AbstractLoss",
        "AbstractSparseBasis",
        "L1Norm",
        "MSELoss",
